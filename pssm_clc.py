...
def download_bxml(fasta_string, protein_name, save_path):
    ...
    with open(save_path+'/'+protein_name + ".xml", "w") as out_handle:        
...
if __name__ == '__main__':
    import pandas as pd
    # inpath = '/Users/suhancho/script/uniprot/ref_codes/bio-protein-fold-recognition/src/'
    inpath = '/Users/suhancho/data/Uniprot_metalbinding_challenge/'
    # protein_dtfrm = pd.read_csv(inpath+"/dataset/EDD/EDD_raw.csv")
    protein_dtfrm = pd.read_table('/Users/suhancho/data/Uniprot_metalbinding_challenge/sequence_df.tsv',sep='\t')
    # dataset_PSSM(protein_dtfrm, inpath+'/BXML/edd_bxml/', inpath+'/PSSM/edd_PSSM/')
    # dl_bxml_dataset(protein_dtfrm, inpath+'/BXML/edd_bxml/')
    m_thread_dl_bxml(protein_dtfrm, inpath+'protein.txt', inpath+'/xml/', 48)
    # fasta_str = fasta_string(protein_dtfrm['Protein name'][1],protein_dtfrm['Protein sequence'][1])
    # download_bxml(fasta_str,protein_dtfrm['Protein name'][1],inpath+'/test/')
    # match, subject = find_aligments(inpath+'/BXML/edd_bxml/'+protein_dtfrm['Protein name'][1] + ".xml", protein_dtfrm['Protein sequence'][1])

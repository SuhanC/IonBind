...
echo "from csv import DictReader
...
def inference_fasta(infile):
    model = keras.models.load_model('/home/suhan/script/IonBind/Saved_Ionbind_NLP_221210/')
...
infiles_lst = open('/home/suhan/data/Uniprot_metalbinding_challenge/testfiles.chunked.txt','r').readlines()
infiles_lst = [f.strip() for f in infiles_lst]

infiles = open(infiles_lst[${i}],'r').readlines()
infiles_final = [f.strip() for f in infiles]

Parallel(n_jobs=64, backend='loky')(delayed(inference_fasta)(i) for i in infiles_final)

">/home/suhan/script/IonBind/inference_runcode/job.${i}.py
